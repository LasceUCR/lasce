...
// Fixture data for the story and its colocated test — `PublicationsExplorer` is
// presentational, so it gets its own sample data rather than depending on
// `getPublications()`, which reads from Postgres. Same four real publications
// this component was demoed with before `/investigacion` moved to the
// database (see `packages/db/prisma/seed.ts` for the live equivalents).
