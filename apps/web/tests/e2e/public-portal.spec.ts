...
test('displays space weather information without authentication', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'El Sol y el clima espacial' })).toBeVisible()
  await expect(page.getByRole('heading', { name: 'El trabajo de LASCE' })).toBeVisible()
  await expect(page.getByRole('img', { name: /Fulguración solar/ }).first()).toBeVisible()
