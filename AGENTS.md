...
`pnpm lint typecheck test` covers `ruff`, `mypy` and `pytest` too — the worker is wired into
Turborepo through a `package.json` that shells out to `uv`.

Requires Node 22+ (`.nvmrc`), pnpm 10, and [uv](https://docs.astral.sh/uv/).

## Git workflow — this is enforced, not advisory
...
- Commits: `<type>(<module>): <description>` — `feat(auth): add user login`. Description starts
  with a verb, lowercase, no trailing period.
...
`no-untyped-call` — your code around them is still checked. The worker runs on asyncio: Postgres
...
- **Mock** the Server Action boundary and `fetch`; for services, mock the **SDK** at the module
  boundary. **Never mock `@lasce/contracts`** — real validation is the entire point of it.
...
- Coverage floors gate every suite, so adding code without tests can break the build even when
  ...
  table in `docs/testing.md`** — that table lags the configs. Floors ratchet upward; raise one
...
Every process reads the single `.env` at the repository root. It is validated **twice** —
`packages/config/src/env.ts` (Zod) and `apps/worker/app/settings.py` (pydantic-settings). Adding a
...
- A required check stuck at _"Expected"_ did not run — usually a workflow renamed without
  updating the ruleset. Raise it; waiting will not help.
